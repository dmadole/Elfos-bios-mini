...
; 1802/Mini configuration ... /DSM
#ifdef 1802MINI
#define IDE_SELECT     2       ; These can all be anything on the 1802/Mini
#define IDE_DATA       3       ;  but conventionally they are ports 2 and 3
#define IDE_SETTLE     500     ; Milliseconds to delay before booting
#define SERP           bn2     ; These can be anything on the 1802/Mini but
#define SERN           b2      ;  conventionally they are the same as on the
#define SERSEQ         req     ;  Pico/Elf using EF2 for serial input and
#define SERREQ         seq     ;  Q for serrial output, both inverted
#define BASE           0f000h  ; High ROM model for the 1802/Mini
#define LAST_RAM       07fffh  ; Assume 32K of RAM instead of searching
...
#ifdef EXPANDER
#define NO_MINIMON             ; Leave out f_minimon call to save BIOS space
#define 72421_RTC              ; Build 72421 RTC code -- needs NO_MINIMON
#define  72421_PORT    5       ;  Set port number for 72421 RTC
#endif
;define INIT_CON               ; Initialize console in f_boot before booting
#endif

; Super Elf configuration ... /DSM
#ifdef SUPERELF
#define IDE_SELECT     2       ; These can all be anything on the
#define IDE_DATA       3       ; 1802/Mini but conventionally they
#define IDE_SETTLE     500     ; Milliseconds to delay before booting
#define SERP           bn2     ; are set to the same as the Pico/Elf
#define SERN           b2
#define SERSEQ         req
#define SERREQ         seq
#define BASE           0f000h
#ifdef EXPANDER
#define LAST_RAM       0f7ffh  ; Assume 62K of RAM instead of searching
#define NO_MINIMON             ; Leave out f_minimon call to save BIOS space
#define 72421_RTC              ; Build 72421 RTC code -- needs NO_MINIMON
#define  72421_PORT    5       ;  Set port number for 72421 RTC
#else
...
#endif
#define BAUD_FIX               ; Use rounding-off to calculate auto-baud
#define SET_BAUD       1200    ; Use this on bit-bang instead of auto-baud
...
; RC1802 configuration ... /DSM
#ifdef RC1802
#define IDE_SELECT     2       ; These can all be anything on the
#define IDE_DATA       3       ; 1802/Mini but conventionally they
#define IDE_SETTLE     250     ; Milliseconds to delay before booting
#define SERP           bn3     ; are set to the same as the Pico/Elf
#define SERN           b3
#define SERSEQ         req
#define SERREQ         seq
#define BASE           0f000h
...
#else
#ifdef 1854_UART
f_utype:   lbr     utype54      ; type console character via UART /DSM
f_uread:   lbr     uread54      ; read console character via UART /DSM
f_utest:   lbr     utest54      ; test UART for character available /DSM
f_usetbd:  lbr     ustbd54      ; set UART baud rate and character format /DSM
#else
f_utype:   lbr     err
f_uread:   lbr     err
f_utest:   lbr     err
...
#else
#ifdef 72421_RTC
f_gettod:  lbr     get72421
f_settod:  lbr     set72421
#else
f_gettod:  lbr     err
f_settod:  lbr     err
#endif
...
; software control on a generic 1854 implementation. /DSM
...
get72421:   sex   r3                    ; output register d address to rtc
            out   72421_PORT
            db    2dh
...
todhold:    out   72421_PORT            ; set hold bit
            db    11h
...
set72421:   glo   rd                    ; save so we can use as table pointer
            stxd
            ghi   rd
            stxd
...
setinit:    out   72421_PORT            ; output values until zero reached
            ldn   rd
            bnz   setinit
...
clocktab:   db    29h                   ; month
            ...
            ; Note that we use XOR to read memory in each case so that on
            ; access to a location with no memory at all on a machine with
            ; a floating bus, we will read the same thing, as bus capacitance
            ; normally causes the opcode before to get read back in this case.
            ...
            glo   re                    ; wait until value just written
...
loop:       glo   rf
            ...
            ghi   rf
            adi   1                     ; advance pointer to next page
	    phi   rf
	    ...
	    xri   255
            ...
            sep   rd
            bnf   loop                    ; if it was ram, keep looping
...
; rate in RE.1 is set to zero, and from the bit-banged UART otherwise. /DSM
...
; if RE.1 bit zero is set by falling through to UTYPE54 after input. /DSM

uread54:   ghi     re
           shr

read54lp:  inp     1854_STATUS
           ani     1
           bz      read54lp
...
; UTYPE54 outputs character in D through 1854 UART. /DSM

utype54:   inp     1854_STATUS
           shl
           bnf     utype54
...
; jumping to UTYPE54, or through bit-banged port otherwise. /DSM
...
; and DF=0 otherwise. /DSM

utest54:   inp     1854_STATUS
           shr
           sep     sret

; End of 1854 UART send and receive code. /DSM
...
timal1:
           ...
           glo     rc
           stxd
           ldi     (((FREQ_KHZ*5)/(SET_BAUD/25)-32)/8)*2+1 ; /DSM
           phi     re
           mov     rd,delay
           ldi     10
           plo     rc
...
end_sb:    SERN    $                   ; wait until start bit found
           SERP    $
#ifdef BAUD_FIX
           sex     r2                  ; when detecting the baud rate
           sex     r2                  ; round instead of truncating /DSM
#endif
setbd1:    inc     rc
           sex     r2
           sex     r2
...
; 8250 UART registers, and we want to be compatible with that. /DSM

ustbd54:  ani     7                   ; mask baud rate bits,
          lsz                         ;  if not zero,
          adi     1                   ;  add one
...
#ifdef LAST_RAM
xfreemem:   ldi     high LAST_RAM       ; don't search for memory,
           phi     rf                  ; just return this value /DSM
           ldi     low LAST_RAM
           plo     rf
#else
#ifdef MCHIP
...
#endif
#ifdef INIT_CON
           ldi     high initcon        ; prepare for seetin call
           phi     r6
           ldi     low initcon
           plo     r6
#else
           ldi     high bootret        ; prepare for seetin call
           phi     r6
           ldi     low bootret
           plo     r6
#endif
